...
async function getOauthToken() {
    const body = {
        ...
        api_key: (0,core.getInput)('api-key')
    };
...
async function deployChanges(token) {
    const body = {
        server_id: (0,core.getInput)('server-id'),
        app_id: (0,core.getInput)('app-id'),
        branch_name: (0,core.getInput)('branch-name'),
        deploy_path: (0,core.getInput)('deploy-path'),
    };
    ...
    return await fetch(`${ apiUri }/git/pull`, options).then(response => {
        return response.json().then(data => {
            (0,core.info)(`Success. Operation ID: ${ response.status }`);
            return {
                ok: response.ok,
...
async function run() {
    try {
        ...
        await deployChanges(oauthToken.access_token).then(response => {
            ...
            (0,core.info)(`Success. Operation ID: ${ response.body.operation_id }`);
            (0,core.setOutput)('operation', response.body.operation_id);
        });
    } catch (error) {
